...
Supabase Free 当前提供 1 GB Storage。Chunk Talk 采用以下限制：

- 录音编码目标约 48 kbps；
- 每条最长 90 秒；
- 单条文件最大 2 MB；
- 每位用户只保留最近 20 条云端录音；
- 保存第 21 条时自动删除该用户最早的一条录音及其数据库记录；
- 本地录音和回放不计入 Supabase Storage，只有点击“保存到账号”才上传。

旧项目需要在 Supabase SQL Editor 执行 `supabase/recording-limits.sql`，把 bucket 的服务端单文件限制同步为 2 MB。Supabase Dashboard 的组织 Usage 页面可以查看 Storage Size 和 Egress；接近 80% 时应清理旧录音或升级套餐。
